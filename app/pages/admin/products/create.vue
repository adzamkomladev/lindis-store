  ...
  <div class="max-w-4xl mx-auto py-6">
    ...
    <div class="flex items-center gap-4 mb-8">
      ...
      <div>
        ...
        <h2 class="font-headline font-black text-on-surface tracking-tighter uppercase text-on-surfacexl">Create Product</h2>
      </div>
    </div>
    ...
    <form @submit="onSubmit" class="space-y-5">
          ...
          <div class="bg-surface-container-lowest border border-outline-variant">
            <div class="flex items-center gap-3 px-6 py-5 border-b border-outline-variant">
              ...
              <div>
                ...
                <p class="text-xs text-on-surface-variant font-body mt-0.5">Product name and description</p>
              </div>
            </div>
            <div class="p-6 space-y-5">
              ...
              <div class="space-y-2">
                ...
                <Textarea v-model="description" v-bind="descriptionAttrs" placeholder="Describe your product..."
                  class="min-h-28 border-outline-variant bg-surface-container-low text-sm font-body focus-visible:ring-primary resize-none" />
              </div>
            </div>
          </div>
            ...
            <div class="p-6">
              <div v-if="uploadedImages.length" class="grid grid-cols-3 gap-3 mb-4">
                <div v-for="(img, idx) in uploadedImages" :key="idx" class="relative aspect-square border border-outline-variant overflow-hidden group">
                  ...
                  <span v-if="idx === 0" class="absolute top-2 left-2 bg-primary text-[#b1c6ff] text-[10px] font-label font-bold uppercase tracking-widest px-2 py-0.5">Primary</span>
                </div>
              </div>
              <label
              ...
              >
                ...
                <Loader2 v-if="isUploading" class="w-7 h-7 text-on-surface-variant animate-spin mb-2" />
                <Upload v-else class="w-7 h-7 text-on-surface-variant mb-2" />
                <p class="text-xs font-label font-bold uppercase tracking-widest text-on-surface-variant">
                  {{ isUploading ? 'Uploading...' : 'Click or drag to upload' }}
                </p>
                <p class="text-[10px] text-on-surface-variant font-body mt-1">PNG, JPG up to 10MB</p>
              </label>
            </div>
          </div>
        ...
        <div class="space-y-5">
          ...
          <div class="bg-surface-container-lowest border border-outline-variant">
            ...
            <div class="p-5 space-y-2">
              <label class="text-xs font-body text-on-surface-variant block">Price (GHS) <span class="text-red-500">*</span></label>
              <div class="relative">
                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant font-body text-sm">₵</span>
          ...
          <div class="bg-surface-container-lowest border border-outline-variant">
            ...
            <div class="p-5 space-y-2">
              <label class="text-xs font-body text-on-surface-variant block">Stock Quantity <span class="text-red-500">*</span></label>
            ...
            <div class="p-5">
              <Select v-model="status">
                ...
                <SelectContent>
                  ...
                  <SelectItem value="active"><span class="flex items-center gap-2"><span class="w-2 h-2 rounded-full bg-primary/50 inline-block" />Active</span></SelectItem>
      ...
      <div class="flex justify-end gap-3 pt-2">
        <NuxtLink to="/admin/products">
          <button type="button" class="px-6 py-3 border border-outline-variant text-on-surface font-label font-bold uppercase text-xs tracking-widest hover:bg-surface-container-low transition-colors">Cancel</button>
        </NuxtLink>
